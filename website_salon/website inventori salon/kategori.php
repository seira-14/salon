...
if (isset($_GET['hapus'])) {
    ...
    $query_hapus = "DELETE FROM kategori WHERE id_kategori = ?";
    $stmt_hapus = $conn->prepare($query_hapus);
    $stmt_hapus->bind_param("i", $id_hapus);

    if ($stmt_hapus->execute()) {
        $_SESSION['success_message'] = "Data kategori berhasil dihapus.";
    } else {
        $_SESSION['error_message'] = "Gagal menghapus data kategori. Mungkin kategori masih digunakan oleh barang.";
    }
...
<head>
    ...
    <title>Data Kategori - Sistem Inventori Salon</title>
                                ...
                                <tbody>
                                    ...
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo "<td>
                                                    <div class='btn-group'>
                                                        <a href='kategori_edit.php?id=" . $row['id_kategori'] . "' class='btn btn-sm btn-warning'>
